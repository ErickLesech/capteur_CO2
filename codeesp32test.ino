...
String format_heure(int x, int y){
   char bufheure[10];
  char bufmin[10];
  char BuferFormat[40];
 if (x<10){
  sprintf(bufmin,"%d%dmm",0,x);
  }else{
  sprintf(bufmin,"%dmm",x);    
  }
  if (y<10){
  sprintf(bufheure,"%d%dh",0,y);
  }else{
  sprintf(bufheure,"%dh",y);   
  }
  strcat(strcpy(BuferFormat,bufheure),bufmin);
  return (BuferFormat);
}
//Fonction setup
void setup()
      ...
      if (i==0){
         //Changer le nom de la salle ici si ce n'est pas la salle correspondante.
            if (asprintf(&num, "%d", count) == -1) {
                ...
                strcat(strcpy(buffer, "Micro_11A/jour1/heure"),num);
              }
               ...
               // sprintf (minute1,"%dH%d", heure,min);     
               String minute1= format_heure(minute, heure);                                             
            ...
            if (asprintf(&num, "%d", count) == -1) {
                ...
                strcat(strcpy(buffer, "Micro_11A/jour1/heure"),num);
                  ...
                  free(num);   
             printf("%s\n", buffer); 
             strcat(strcpy(buffer, buffer),"/min1/taux_de_CO2");
        ...
        //  deuxieme part de l'heure 
         ...
         //inserer le jour 
            ...
            strcat(strcpy(buffer, buffer),"/min2/time");
               ...
               String minute2= format_heure(minute,heure);  
        ...
        // inserer l'heure 
             ...
             strcat(strcpy(buffer, buffer),"/min2/taux_de_CO2");
             if (Firebase.RTDB.setInt(&fbdo,buffer,concentration)){
              Serial.println("PASSED");
              Serial.println("PATH: " + fbdo.dataPath());
              Serial.println("TYPE: " + fbdo.dataType());
        ...
        if (i==2){
         //inserer le jour 
            ...
            String minute3= format_heure(minute,heure);  
        ...
        // inserer l'heure 
             ...
             if (asprintf(&num, "%d", count) == -1) {
                ...
                strcat(strcpy(buffer, "Micro_11A/jour1/heure"),num);
                  ...
                  free(num);   
             printf("%s\n", buffer); 
             strcat(strcpy(buffer, buffer),"/min3/taux_de_CO2");  
         ...
         //inserer le jour 
            ...
            strcat(strcpy(buffer, buffer),"/min4/time");
               ...
               String minute4 = format_heure(minute,heure);  
        ...
        // inserer l'heure 
              ...
              if (asprintf(&num, "%d", count) == -1) {
                ...
                strcat(strcpy(buffer, "Micro_11A/jour1/heure"),num);
                  ...
                  free(num);   
                 printf("%s\n", buffer); 
                 strcat(strcpy(buffer, buffer),"/min4/taux_de_CO2");
             if (Firebase.RTDB.setInt(&fbdo,buffer,concentration)){
              Serial.println("PASSED");
              Serial.println("PATH: " + fbdo.dataPath());
              Serial.println("TYPE: " + fbdo.dataType());
